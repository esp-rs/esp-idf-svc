...
use crate::sys::{
    ...
    otSrpClientSetTtl, otSrpClientStart, otSrpClientStop, EspError, ESP_ERR_INVALID_STATE,
};
    ...
    T: Mode,
{
        ...
        F: FnOnce(&SrpConf, SrpState, bool) -> Result<R, EspError>,
    {
        ...
        f(&conf, info.mState.into(), !self.inner().srp.conf_taken)
    }
    ...
    pub fn srp_is_empty(&self) -> Result<bool, EspError> {
        ...
        let srp = &mut self.inner().srp;

        Ok(!srp.conf_taken && srp.services.iter().all(|service| !service.taken))
    }
    ...
    pub fn srp_set_conf(&self, conf: &SrpConf) -> Result<(), EspError> {
        ...
        conf.store(&mut srp_conf, &mut srp.conf_buf)?;
        srp.conf_taken = true;
    ...
    pub fn srp_autostart(&self) -> Result<(), EspError> {
        ...
        unsafe {
            otSrpClientEnableAutoStartMode(
                ...
                &mut self.inner().srp as *mut _ as *mut _,
            );
        }
        ...
        TI: Iterator<Item = (&'a str, &'a [u8])> + Clone + 'a,
    {
        ...
        let slot = srp.services.iter().position(|service| !service.taken);
        ...
        let our_service = &mut srp.services[slot];
    ...
    pub fn srp_remove_service(
        ...
        if slot > srp.services.len() || !srp.services[slot].taken {
            ot_esp!(otError_OT_ERROR_INVALID_ARGS)?;
        }

        let service = &mut srp.services[slot];
    ...
    pub fn srp_remove_all(&self, immediate: bool) -> Result<(), EspError> {
        ...
        if immediate {
            ...
            for service in &mut srp.services {
                service.taken = false;
            }
...
impl OtSrp {
    pub(crate) unsafe fn init(this: *mut Self) {
        unsafe {
            ...
            addr_of_mut!((*this).conf_buf).write_bytes(0, SRP_HOST_BUF_SIZE);
    ...
    pub(crate) unsafe extern "C" fn plat_c_srp_state_change_callback(
        ...
        let srp = context as *mut alloc::boxed::Box<OtSrp>;
...
impl OtSrpService {
    pub(crate) unsafe fn init(this: *mut Self) {
        unsafe {
            ...
            addr_of_mut!((*this).buf).write_bytes(0, SRP_SVC_BUF_SIZE);
            addr_of_mut!((*this).service)
                .write_bytes(0, core::mem::size_of::<otSrpClientService>());
        }
    }
}
